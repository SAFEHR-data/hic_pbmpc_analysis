...
df_device_exposure_transfusion <- df_device_exposure_transfusion %>%
   ...
   mutate(age_at_transfusion = 
             ceiling(as.numeric(difftime(device_exposure_start_date, birth_datetime, units = "days")) / 365)) %>%
  select(
    person_id,
...
# This is not the total quantity by age or year since you're grouping by the device exposure start date
df_total_quantity_age <- df_device_exposure_transfusion %>% 
   group_by(person_id,age_at_transfusion,device_exposure_start_date) %>% 
   summarise(
...
# What about removing this bit? I don't think you need this.
# %>%
#    left_join(df_total_quantity_age, by="person_id","device_exposure_start_date=device_exposure_start_date", "age_at_transfusion") 
...
#  Number of RBC transfused to patients  ≥65 year's of age
 ...
 df_age_window <- df %>%
   ...
   summarise(ist_age= min(age_at_transfusion))
