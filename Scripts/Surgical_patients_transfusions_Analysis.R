...
# Extract unique patient IDs who had device exposures
df_device_exposure_person_id <- df_device_exposure |>
  select(person_id) |>
  distinct()
...
df_transfusion_7days <- df_procedure_occurrence |>
  inner_join(df_device_exposure, by = "person_id", relationship = "many-to-many") |>
  mutate(
    within_7_days = case_when(
      device_exposure_start_date >= procedure_date &
        device_exposure_start_date <= procedure_date + days(7) ~ TRUE,
      TRUE ~ FALSE
    )
  ) |>
  filter(within_7_days==TRUE) %>% distinct()

df_transfusion_7days <- df_transfusion_7days |>
  select(
    person_id,
...
summary_by_procedure <- df_transfusion_7days |>
  ...
  summarise(
    ...
    units_received = sum(quantity.x, na.rm = TRUE),
    .groups = "drop"
  ) |>
